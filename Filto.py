...
find_element(element_name):滑动等待元素出现
"""


# import os
# os.path.isfile('test.txt') #如果文件不存在就返回False
...
def open_setting(device_name):
    """从首页进入设置页"""
    d(resourceId="com.video.editor.filto:id/ll_home_bottom_setting").click()  # 打开设置
    sleep(1)
    Screenshot_img(device_name, "设置")
    d(resourceId="com.video.editor.filto:id/ll_home_bottom_discover").click()  # 返回发现页
    print("回到首页")
    sleep(0.5)
...
def open_Subscribe(device_name):
    """从首页进入订阅面"""
    d(resourceId="com.video.editor.filto:id/iv_home_pro_icon").click()  # 点击首页 订阅入口
    sleep(1)
...
def join_edit(device_name):
    ...
    while True:
        if d(resourceId='com.video.editor.filto:id/media_tab_tv').exists:  # 判断是否加载完成 返回ture
            ...
            d.xpath('//*[@resource-id="com.video.editor.filto:id/rl_media"]/android.view.ViewGroup[1]').click()  # 点击相册页的第一个元素
            sleep(1)
...
def join_share(device_name):
    """分享页"""
    while True:
        if d(resourceId="com.video.editor.filto:id/iv_share_back_home").exists:# 判断是否保存完成
            sleep(3)
            if d(resourceId="com.video.editor.filto:id/tv_rate_us_content").exists:
                print("fff")
                Screenshot_img(device_name, "引导评论弹框")
...
def find_element_filter(element_name):
    #查找滤镜
    while True:
        if d(text=element_name).wait(timeout=1.0):
            print(element_name+"出现")
            return True
        else:
            d(resourceId="com.video.editor.filto:id/vp_media_filter").swipe("left", steps=20)
            continue


def find_element_effect(element_name):
    #查找特效
    while True:
        if d(text=element_name).wait(timeout=1.0):
            print(element_name+"出现")
            ...
            d(resourceId="com.video.editor.filto:id/vp_edit_effect").swipe("left", steps=20)
            continue
